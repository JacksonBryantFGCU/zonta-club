...
export default function AdminDashboard() {
  ...
  const filteredOrders = useMemo(() => {
    ...
    if (search.trim()) filtered = filtered.filter((o) => o.email.toLowerCase().includes(search.toLowerCase()));
    const now = new Date();
    if (dateRange === "7") {
  ...
  if (isError)
    return <p className="text-center py-20 text-red-600">Failed to load orders.</p>;
    ...
    <section className="py-16 px-6 max-w-7xl mx-auto relative">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-10">
        <div className="bg-white shadow-md rounded-xl border border-zontaGold p-6 text-center">
          <h3 className="text-zontaDark text-sm font-semibold mb-2">Total Revenue</h3>
          <p className="text-2xl font-bold text-zontaRed">${stats.totalRevenue.toFixed(2)}</p>
        </div>
        <div className="bg-white shadow-md rounded-xl border border-zontaGold p-6 text-center">
        ...
        <div className="bg-white shadow-md rounded-xl border border-zontaGold p-6 text-center">
          <h3 className="text-zontaDark text-sm font-semibold mb-2">Completed</h3>
          <p className="text-2xl font-bold text-green-600">{stats.completed}</p>
        </div>
        <div className="bg-white shadow-md rounded-xl border border-zontaGold p-6 text-center">
          <h3 className="text-zontaDark text-sm font-semibold mb-2">Cancelled</h3>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 mb-10">
        <div className="bg-white border border-zontaGold rounded-xl shadow-md p-6">
          <h3 className="text-lg font-semibold text-zontaRed mb-4">Monthly Revenue</h3>
        ...
        <div className="bg-white border border-zontaGold rounded-xl shadow-md p-6">
          <h3 className="text-lg font-semibold text-zontaRed mb-4">Orders by Status</h3>
          <ResponsiveContainer width="100%" height={300}>
            <PieChart>
      ...
      {filteredOrders.length === 0 ? (
        ...
        <>
            ...
            <table className="w-full border-collapse border border-zontaGold text-left">
              <thead className="bg-zontaGold text-white">
                <tr>
                  ...
                  <th className="px-8 py-2 text-left">Actions</th>
                </tr>
              </thead>
              <tbody>
                {filteredOrders.map((order: Order) => (
                  <tr key={order._id} className="border-b border-zontaGold/40 hover:bg-zontaGold/10">
                    ...
                    <td className="px-8 py-2">
                      <div className="flex items-center gap-2">
                        ...
                        <button
                          ...
                          className="bg-zontaGold text-white px-3 py-1 rounded-md text-sm font-semibold hover:bg-zontaDark transition"
                        >
                          View Details
                        </button>
                      </div>
          ...
          <div className="block md:hidden space-y-4">
            {filteredOrders.map((order: Order) => (
              <div key={order._id} className="bg-white border border-zontaGold rounded-lg shadow-sm p-4">
                <div className="flex justify-between items-center mb-2">
                  <span className="text-sm font-semibold text-zontaRed">{order.email}</span>
                  <span
