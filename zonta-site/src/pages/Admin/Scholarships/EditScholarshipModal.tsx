...
interface EditScholarshipModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (id: string, scholarship: ScholarshipFormData) => void;
...
export default function EditScholarshipModal({
  ...
  useEffect(() => {
    if (isOpen && scholarship) {
      // Convert deadline to date string format
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (scholarship) {
      onSubmit(scholarship._id, formData);
    }
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-2xl mx-4 max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          <h2 className="text-2xl font-bold text-zontaRed">Edit Scholarship</h2>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          <div>
            <label
              htmlFor="editScholarshipDeadline"
              className="block text-sm font-semibold text-gray-700 mb-2"
            >
              Application Deadline
            </label>
            <input
              id="editScholarshipDeadline"
              type="date"
